...
int detectDeadlock(){
	int i, j, k;
	int pushflag=0, popflag;

	int remember=0;
	int* rememArr=(int*)malloc(sizeof(int)*numProcess);
	for(i=0;i<numProcess;i++) rememArr[i]=-1;
	...
	while(deadlock==0){
		for(i=0; i<numProcess; i++){
			//find next process
			if(possibleProcess[i]!=0 || (top!=-1 && (possibleProcess[i]==top+1 || possibleProcess[i]==-(top+1)))) continue;
		...
		if(i==numProcess) {
			...
			if(top==-1) {
				...
				for(j=0; j<numProcess; j++){
					if(possibleProcess[j]<=0) {
						deadlockedProcess[j]=1;

						//not deadlock state process because of last stack trace
						int tmp;
						for(k=0; k < -remember; k++){
							tmp=rememArr[k];
							deadlockedProcess[tmp]=0;
						}
					}
				}
				break;
			...
			//pop and find another way
			else{
				if((popflag=pop())==-1) return -1;
				
				//remember the last big stack
				if(-(top+2) < remember){
					remember=-(top+2);
					
					for(k=0;k<-remember-1;k++){
						rememArr[k]=stack[k];
					}
					rememArr[k]=popflag;
				}

				printf("pop(pid : %d, remember : %d)  ", popflag, remember);//^
				continue;
			}
		}
...
int pop(){
	...
	// Stack pop
		...
		ret=stack[top];
		possibleProcess[ret]=-(top+1);
		top--;

		for(b=0;b<numRsrcType;b++){
			available[b] -= allocation[ret][b];
		}
	}
